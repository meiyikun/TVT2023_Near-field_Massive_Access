%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NMSE performance vs Number of High-resolution ADCs
% Only TS-OAMP
% 2 bits quantization
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear;
...
%% Initialization
alg_set = [1:8];  
alg_type = length(alg_set);
var_set = [50,100,200];
Nsim = ones(1,length(var_set))*500;
method = char('TS-OAMP');
...
for i_var = 1:length(var_set)
    ...
    for i_sim = 1:Nsim(i_var)
        ...
        for alg_sel_ii = 1:length(alg_set)
            ...
            switch alg_sel
                case {1}  % number of High-resolution ADCs = 0
                    para.intvl = +inf;
                    para.Num_mix = 0;
                ...
                case {2}  % number of High-resolution ADCs = 4
                    para.intvl = 128;
                ...
                case {3}  % number of High-resolution ADCs = 16
                    para.intvl = 32;
                    para.Num_mix = para.Nr/ para.intvl;                     % total number of mixed-ADCs
                    para.Num_mix_sub = para.Num_mix/para.Num_sub;
                    mix_index = para.intvl/2:para.intvl:para.Nr;            % uniformly distributed  
                ...
                case {4}  % number of High-resolution ADCs = 32
                    para.intvl = 16;
                    para.Num_mix = para.Nr/ para.intvl;                     % total number of mixed-ADCs
                    para.Num_mix_sub = para.Num_mix/para.Num_sub;
                    mix_index = para.intvl/2:para.intvl:para.Nr;            % uniformly distributed  
                ...
                case {5}  % number of High-resolution ADCs = 64
                    para.intvl = 8;
                    para.Num_mix = para.Nr/ para.intvl;                     % total number of mixed-ADCs
                    para.Num_mix_sub = para.Num_mix/para.Num_sub;
                    mix_index = para.intvl/2:para.intvl:para.Nr;            % uniformly distributed  
                ...
                case {6} % number of High-resolution ADCs = 128
                    para.intvl = 4;
                    para.Num_mix = para.Nr/ para.intvl;                     % total number of mixed-ADCs
                    para.Num_mix_sub = para.Num_mix/para.Num_sub;
                    mix_index = para.intvl/2:para.intvl:para.Nr;            % uniformly distributed  
                ...
                case {7}  % number of High-resolution ADCs = 256
                    para.intvl = 2;
                    para.Num_mix = para.Nr/ para.intvl;                     % total number of mixed-ADCs
                    para.Num_mix_sub = para.Num_mix/para.Num_sub;
                    mix_index = para.intvl/2:para.intvl:para.Nr;            % uniformly distributed  
                ...
                case {8} % number of High-resolution ADCs = 512
                    para.intvl = 1;
                    para.Num_mix = para.Nr/ para.intvl;                     % total number of mixed-ADCs
                    para.Num_mix_sub = para.Num_mix/para.Num_sub;
                    mix_index = 1:para.Nr;                                  % uniformly distributed       
                    Y_wave(:,mix_index) = Y_norm(:,mix_index);              % high-resolution ADCs with infinite quantization
            ...
            if mod(i_sim,min(10,Nsim(i_var))) == 0 %|| i_sim == 1
                fprintf('sim=%d, Pt=%d dBm, M=%d, Num of H-ADCs=%d, %s: AER=%.6f, AER_tmp=%.6f, NMSE=%.2f dB, NMSE_tmp=%.2f dB, Pfa=%.4f, Pmd=%.4f \n',...
                    i_sim, para.Ps_dBm, para.M, para.Nr/para.intvl, method, AER(alg_sel, i_var)/i_sim, AER_tmp,...
...
% save GainvsNumofHADC_Bit2.mat Perform;
%% Plot
MarkerSize = 6;
LineWidth = 2;
Fontsize = 15;
figure
var_set2 = [0,4,16,32,64,128,256,512];
var_set3 = [0:7];
p1=plot(var_set3,Perform.NMSE(:,1,end),'k-<','MarkerFaceColor',[0 0 0],'MarkerSize',MarkerSize,'LineWidth',LineWidth);grid on;hold on;
p2=plot(var_set3,Perform.NMSE(:,2,end),'g-*','MarkerFaceColor',[0 0 0],'MarkerSize',MarkerSize,'LineWidth',LineWidth);grid on;hold on;
p3=plot(var_set3,Perform.NMSE(:,3,end),'b-s','MarkerFaceColor',[0 0 0],'MarkerSize',MarkerSize,'LineWidth',LineWidth);grid on;hold on;
set(gca,'xtick',var_set3,'xticklabel',var_set2,'FontSize',Fontsize);
gcf_set = [p1,p2,p3];
legend(gcf_set,{'$M = 50, P_t = 10$ dBm','$M = 100, P_t = 10$ dBm','$M = 200, P_t = 10$ dBm'},'interpreter','latex','location','southwest');  
xlabel('Number of high-resolution ADCs $|{\cal H}|_c$','Fontsize',Fontsize,'interpreter','latex');
