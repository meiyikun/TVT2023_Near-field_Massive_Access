function [z_post, vz_post] = mmse_estimator(y, z, mz, sigma, NumBits, delta, para)
% Performs output node update, i.e., component-wise MMSE estimate of
% quantization
%
...
r_low = y - delta/2;
% r_low(r_low < -(2^NumBits-1/2)*delta) = -(2^NumBits-1/2)*delta;  % -1e50
r_low(r_low <= -(2^NumBits-1/2)*delta) = -1/para.adj/sqrt(2);
% r_low(r_low <= -(2^NumBits-1/2)*delta) = -2;

r_up = y + delta/2;
% r_up(r_up > (2^NumBits-1/2)*delta) = (2^NumBits-1/2)*delta;
r_up(r_up >= (2^NumBits-1/2)*delta) = 1/para.adj/sqrt(2);
% r_up(r_up >= (2^NumBits-1/2)*delta) = 2;
