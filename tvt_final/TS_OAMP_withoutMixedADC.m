function [miu_storage, act_hat, thegma2, v_s, para] = TS_OAMP_withoutMixedADC(Y_wave, S, codebook, para, D_sub)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Y_wave: Received signals with mixed-resolution quantization
% S: Pilot matrix
% codebook : Quantization codebook
% para: System parameters
% D_sub: Piecewise angular-domain dictionary
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% initialization 
M = size(Y_wave,1);     Nr = size(Y_wave,2);    K = size(S,2);
damping = para.damping;
niter = para.niter;
Num_sub = para.Num_sub;
Nr_sub = Nr/Num_sub;
Gr = size(D_sub,2);
beta = size(D_sub,2)/size(D_sub,1);
...
lambda_s = repmat(lambda0,K,Nr);
lambda_a = repmat(lambda0,K,Gr);
thegma2 = norm(Y_wave,'fro')^2/101/M/Nr;
% thegma2 = para.Nvar2;
Psi = zeros(1,Nr);
for j = 1:Nr
    Psi(j) = (norm(Y_wave(:,j),2)^2-M*thegma2)/norm(S,'fro')^2/lambda0;
end
...
for iter=1:niter
    if iter<=para.T1
        ...
        thegma2 = mean(abs(Y_wave-S*miu).^2+gamma_mean.','all').';          % Noise variance
    else
        %% Stage 2
        % Initialization 
        ...
        if iter == (para.T1+1)
            u = u*D_sub;
            u_prev_a =  u_prev_s*D_sub;
            v_prev_a = D_sub'*diag(v_prev_s)*D_sub;
            v_prev_a = real(diag(v_prev_a));
            
            Psi = D_sub'*diag(Psi)*D_sub;
            Psi = real(diag(Psi))';
            
            z_ext = z_ext*D_sub;
            nu_ext = D_sub'*diag(nu_ext)*D_sub;
